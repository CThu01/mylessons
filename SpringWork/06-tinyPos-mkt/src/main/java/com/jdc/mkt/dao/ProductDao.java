...
public class ProductDao implements ReferenceServices<Product>{
	...
	public Product findById(int id) {
		...
		try(var con = getConnection();var ps= con.prepareStatement(sql)){
			...
			while(rs.next()) {
				...
				p.setDetialPrice(rs.getInt("p.dt_price"));
				p.setWholeSalsePrice(rs.getInt("p.ws_price"));
				p.setCategory(c);
	...
	public List<Category> findByCategoryNameLike(String catName){
		...
		try(var con = getConnection();var ps= con.prepareStatement(sql)){
			...
			ps.setString(1, catName.toLowerCase().concat("%"));
			var rs = ps.executeQuery();
